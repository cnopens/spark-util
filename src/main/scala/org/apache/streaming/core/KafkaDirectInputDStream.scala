...
class KafkaDirectInputDStream[K: ClassTag, V: ClassTag](
    ...
    var topics: Set[String]) extends KafkaDynamicDStream[K, V] {
  /**
   * @author LMQ
   * @time 20190613
  ...
  private val maxRateLimitPerPartition: Int = {
    val maxRate = ssc.sc.sparkcontext.getConf.getInt("spark.streaming.kafka.maxRatePerPartition", 0) /// (ssc.emptyDataWaitTime.milliseconds / 1000).toInt)
    if (maxRate > 0) Math.max(1, maxRate / (ssc.emptyDataWaitTime.milliseconds / 1000).toInt)
    else maxRate
  }
  /**
   * @author LMQ
   ...
   *
   */
  val maxLimitPerPartition = ssc.sc.sparkcontext.getConf.getLong("spark.streaming.kafka.maxRatePerPartition", 0)
  ...
  override def batchRDD() = {
    ...
    val kafkardd = ssc.sc.kafkaRDD[K, V](topics, currentOffsets, maxMessagesPerPartition(lastOffset))
  ...
  def maxMessagesPerPartition(
    ...
    if (effectiveRateLimitPerPartition.values.sum > 0) {
      ...
      Some(effectiveRateLimitPerPartition.map {
        case (tp, limit) =>
          if (maxLimitPerPartition > 0) tp -> Math.min(maxLimitPerPartition, (secsPerBatch * limit).toLong)
