...
class StreamingDynamicContext {
   ...
   */
  def createKafkaDstream[K: ClassTag, V: ClassTag, KD <: Decoder[K]: ClassTag, VD <: Decoder[V]: ClassTag, R: ClassTag](
    topics: Set[String], msghandle: (MessageAndMetadata[K, V]) => R) = {
    val r = new KafkaDirectInputDStream[K, V, KD, VD, R](this, msghandle, topics)
    this.kafkaDstream = r
    r
  }
}